...
def parse_skeleton(filename, builder, filter):
    """Parse skeleton XML file and add links/joints to builder. Returns node_map, xform_map, mesh_map."""
    node_map = {}       # map node names to link indices
    xform_map = {}      # map node names to parent transforms
    mesh_map = {}       # map mesh names to link indices objects

    file = ET.parse(filename)
    root = file.getroot()
    
    coord_start = len(builder.joint_q)
    dof_start = len(builder.joint_qd)
    ...
    for child in root:
        if (child.tag == "Node"):
            ...
            if parent in node_map:
                parent_link = node_map[parent]
                parent_X_s = xform_map[parent]
            else:
                parent_link = -1
            ...
            xform_map[name] = joint_X_s
            node_map[name] = link
            mesh_map[mesh_base] = link
    
    return node_map, xform_map, mesh_map
